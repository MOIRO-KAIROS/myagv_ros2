   ...
   /* std::cout << "readSpeed: " << ret;
	...
	if ((buf[0] + buf[1] + buf[2] + buf[3]) == buf[4]) {
        ...
        for (int i = 0; i < 4; ++i) {
            if (buf[i] == 1) {
                wheel_num = i+1;
                RCLCPP_ERROR("ERROR %d wheel current > 2000", wheel_num);
            }
        }
        restoreRun();
        return false;
    ...
    // }
    // if (!header_ok) {
    //     //RCLCPP_ERROR("Cannot find header");
    //     return false;
    // }
    ...
    // if (index == 40 - 18)
    // {
    //     RCLCPP_ERROR("Received message header error!");
    //     //return false;
    // }
        ...
        while (true) {
            ...
            if (buf_header[0] == 0x01)
            {
                ...
                if (buf_header[0] == 0x01)
                {
                    ...
                    RCLCPP_INFO("Voltage: %d", msg.data);
                    pub_v.publish(msg);
                    break;
                }
            }
            ...
            {
                RCLCPP_ERROR("Get Voltage timeout");
                break;
            }
        }
    }else{
...
void MyAGV::publisherImuSensor()
{
	...
	ImuSensor.header.stamp = rclcpp::Time::now(); 
...
void MyAGV::publisherImuSensorRaw()
{
	...
	ImuSensorRaw.header.stamp = rclcpp::Time::now(); 
...
{   
    ...
    msgl.header.stamp = rclcpp::Time::now();
